                    ...
                    me.find('input[require], select[require], textarea[require]').each(function() {
                        ...
                        if ($cu.is(':visible')) {
                            ...
                            if ($cu.attr('require') == 'password') {
                                if ($cu.isPasswdErr()) {
                                    txt.push(label + '：密碼長度必須為 6~18 碼，且至少包含英數字');
                                }
                            }
                        ...
                        if (txt.length > 0) {
                            chk = 0;
                            $cu.closest('.form-group').addClass('has-error');
                            g.alert({
                                title: 'Error!',
                                txt: txt.join("\r\n")
                            });
                            ...
                            $cu.closest('.form-group').removeClass('has-error');
                        }
                    });
                ...
                load: function(functionName, sf) {
                    ...
                    if (!g.check(functionName)) {
                        if (typeof importScripts == 'function') {
                            ...
                            importScripts(dir + subfloder + '/' + functionName.toLowerCase() + '.js');
                        } else {
                            g.clog('start scripttag::');
                            $('body').append('<script src="' + dir + subfloder + '/' + functionName.toLowerCase() + '.js">\x3C/script>');
                        }
                    }
                },
