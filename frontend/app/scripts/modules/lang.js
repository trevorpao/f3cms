;(function(app, gee, $){
   ...
   var cfg = {
    ...
    var tsSwitch = {
        ...
        thisTxt: function ( txt )
        {
            if( txt === '' || txt === null ){
                return '';
            }

            if( tsSwitch.currentEncoding === 1 && tsSwitch.targetEncoding === 2 ) {
                return tsSwitch.Simplized(txt);
            }
            else if ( tsSwitch.currentEncoding === 2 && tsSwitch.targetEncoding === 1 ) {
                return tsSwitch.Traditionalized(txt);
            }
            else {
                return txt;
            }
        ...
        thisArea: function ( fobj )
        {
            ...
            for(var i=0;i<objs.length;i++) {
                ...
                if( '||BR|HR|TEXTAREA|'.indexOf( '|'+obj.tagName+'|' ) > 0 || obj === tsSwitch.translateButtonObject ) {
                    continue;
                }
                if( obj.title && obj.title !== '' && obj.title !== null ) {
                    obj.title = tsSwitch.thisTxt( obj.title );
                }
                if( obj.alt && obj.alt !== '' && obj.alt !== null ){
                    obj.alt = tsSwitch.thisTxt( obj.alt );
                }
                if( obj.tagName === 'INPUT' && obj.value !== '' && obj.type !== 'text' && obj.type !== 'hidden' ) {
                    obj.value = tsSwitch.thisTxt( obj.value );
                }
                if( obj.nodeType === 3 ){
                    obj.data = tsSwitch.thisTxt( obj.data );
                }
                else {
                    tsSwitch.thisArea(obj);
                }
            }
        },
        ...
        thisPage: function ()
        {
            if( tsSwitch.targetEncoding === 1 ) {
                // tsSwitch.translateButtonObject.innerHTML = tsSwitch.msgToTraditionalChinese;
                app.arena.feed.setItem('pageJSEndcode', tsSwitch.targetEncoding).catch( gee.clog );
                tsSwitch.thisArea();
            }
            else if( tsSwitch.targetEncoding === 2 ) {
                // tsSwitch.translateButtonObject.innerHTML = tsSwitch.msgToSimplifiedChinese;
                app.arena.feed.setItem('pageJSEndcode', tsSwitch.targetEncoding).catch( gee.clog );
                tsSwitch.thisArea();
            }
        },
        ...
        Traditionalized: function ( cc ) {
            var str = '';
            var ss = tsSwitch.JTPYStr();
            var tt = tsSwitch.FTPYStr();
    ...
    app.lang = {
        ...
        init: function() {
            app.arena.feed.getItem('cuLang', function(err, val){
                ...
                app.lang.cu = ($('#lang').val()) ? $('#lang').val() : lang;
                app.lang.set(app.lang.cu);

                if (app.lang.cu === 'tw') {
                    app.lang.initTSSwitch();
                }
                else {
        ...
        initTSSwitch: function () {
            app.arena.feed.getItem('pageJSEndcode', function(err, val){
                ...
                if (val) {
                    tsSwitch.targetEncoding  = val*1;
                }
                else {
                    tsSwitch.targetEncoding  = 1;
                }
